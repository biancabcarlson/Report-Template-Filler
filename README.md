...
The web demo uses the suite-wide investigation interface with a desktop-first split between report inputs and the live output document. Existing template fields, editable document and transaction lists, live transaction totals, tool-output paste blocks, undo behavior, preview generation, and submission confirmation remain intact. The input/output columns stack cleanly on mobile.
