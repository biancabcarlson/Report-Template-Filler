...
Input (template.json) — SYNTHETIC EXAMPLE:
{
  ...
  "sections": [
    ...
    {"heading": "Incident Window", "fields": ["incident_start", "incident_end"]},
...
Input (answers.json) — SYNTHETIC EXAMPLE:
{
  ...
  "incident_start": "2026-08-29",
  "incident_end": "2026-08-31",
  ...
  "transactions_captured": "Aug 31 ACH Withdrawal $4,800.00 (Ref ACH-88300, pending); Aug 31 Crypto Withdrawal $4,590.00 (new wallet, not on allowlist)",
  "transactions_total": "$9,390.00",
  "documents_collected": "August 2026 account statement; ID on file (CA Driver's License, IDVerify Co.); account agreement (signed Jan 14, 2025)",
  "outcome": "Confirmed — ATO",
  "next_steps": "Secure the account and correct any changed PII with the true positive's (TP's) information."
}

Note: the web demo renders Transactions Captured as a fixed, non-editable
totals block (see index.html) rather than free-text fields, since it's a
running total pulled straight from the case's transaction data. The CLI
has no concept of "fixed" vs. "editable" fields — it's fully generic — so
the example above models the same section as two ordinary text fields
instead. Swap in whatever fields fit your own template.
"""
